...
int main() {
    ...
    printf("Enter productions (use format A->XYZ and # for epsilon): \n");
    for (i = 0; i < prod_count; i++) {
        fgets(productions[i], MAX, stdin);
        productions[i][strcspn(productions[i], "\n")] = '\0';
    }

    // IMPORTANT: Initialize FIRST and FOLLOW arrays to empty
...
void findFirst(char *result, char c) {
    ...
    for (int i = 0; i < prod_count; i++) {
        if (productions[i][0] == c) {
            int rhs = 3;
            while (productions[i][rhs] != '\0') {
                char next = productions[i][rhs];
...
void findFollow(char *result, char c) {
    if (follow_visited[c - 'A']) return;
    follow_visited[c - 'A'] = 1;

    if (productions[0][0] == c) addToResult(result, '$');
